#!/usr/bin/env bash
#
# Build the self-contained HTTP/3 native bundle for ioxide.h3: nghttp3 (sans-I/O H3 + QPACK,
...
#
#   scripts/build-h3-native.sh                 # build with pinned refs below
#   NGHTTP3_REF=v1.6.0 scripts/build-h3-native.sh
#
set -euo pipefail
cd "$(dirname "$0")/.."
...
OUT=src/ioxide.h3/runtimes/linux-x64/native
...
SHIM="$OLDPWD/src/ioxide.h3/native/ioxide_h3_shim.c"
gcc -c -O2 -fPIC -o shim.o "$SHIM" \
    -Inghttp3/lib/includes -Inghttp3/build/lib/includes

echo "==> linking libioxide_h3.so"
gcc -shared -o libioxide_h3.so shim.o \
    -Wl,--whole-archive nghttp3/build/lib/libnghttp3.a -Wl,--no-whole-archive \
    -Wl,--no-undefined

echo "==> verifying"
ldd libioxide_h3.so | grep -vE 'vdso|libc\.|ld-linux' && {
    echo "unexpected dependency"; exit 1; } || true
[ "$(nm -D --defined-only libioxide_h3.so | grep -c ' ih3_')" -ge 8 ] || {
    echo "shim exports missing"; exit 1; }

cd - >/dev/null
cp "$WORK/libioxide_h3.so" "$OUT/libioxide_h3.so"
echo "==> done: $OUT/libioxide_h3.so ($(du -h "$OUT/libioxide_h3.so" | cut -f1))"
