#!/usr/bin/env bash
#
# Build the self-contained QUIC native bundle for ioxide.quic:
...
#
#   scripts/build-quic-native.sh                 # build with pinned refs below
#   NGTCP2_REF=v1.14.0 scripts/build-quic-native.sh
#
set -euo pipefail
cd "$(dirname "$0")/.."
...
OUT=src/ioxide.quic/runtimes/linux-x64/native
...
echo "==> verifying"
ldd libioxide_quic.so | grep -vE 'vdso|libcrypto|libc\.|ld-linux' && {
    echo "unexpected dependency"; exit 1; } || true
[ "$(nm -D --defined-only libioxide_quic.so | grep -c ' ngtcp2_')" -gt 500 ] || {
...
echo "==> done: $OUT/libioxide_quic.so ($(du -h "$OUT/libioxide_quic.so" | cut -f1))"
