...
public sealed unsafe class Ring : IDisposable 
{
    ...
    public static Ring Create(uint entries) 
    {
        ...
        // EXPERIMENT: SINGLE_ISSUER + DEFER_TASKRUN are OFF so multiple threads may submit/enter.
        // (NOTE: this only lifts the *kernel* restriction — userspace SQ access must still be locked.)
    ...
    // Lockless pure wait+reap for the reactor: submit nothing (producers submit under the reactor's
    // lock via SubmitAndWait(0)), just block until >=1 completion is available.
    public int Wait() => io_uring_enter(_fd, 0, 1, IORING_ENTER_GETEVENTS);
