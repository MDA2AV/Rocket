/*
 * ioxide.quic native shim: a thin, C#-friendly facade over ngtcp2 + its picotls crypto backend.
 *
                                  ...
                                  void *user_data, void *stream_user_data)
{
    ...
    /* Consume connection + stream flow-control credit for what we just took. */
                         ...
                         iq_callbacks cbs)
{
    ...
    if (ptls_load_certificates(&e->ptls_ctx, cert_pem_path) != 0) {
        fprintf(stderr, "[ioxide.quic] failed to load certificates from %s\n", cert_pem_path);
        goto fail;
    }

    {
        ...
        if (bio == NULL) {
            fprintf(stderr, "[ioxide.quic] failed to open key %s\n", key_pem_path);
            goto fail;
        }
        EVP_PKEY *pkey = PEM_read_bio_PrivateKey(bio, NULL, NULL, NULL);
        BIO_free(bio);
        if (pkey == NULL) {
            fprintf(stderr, "[ioxide.quic] failed to parse key %s\n", key_pem_path);
            goto fail;
        }
        int rv = ptls_openssl_init_sign_certificate(&e->sign_cert, pkey);
        EVP_PKEY_free(pkey);
        if (rv != 0) {
            fprintf(stderr, "[ioxide.quic] failed to init sign_certificate\n");
            goto fail;
        }
    ...
    if (ngtcp2_crypto_picotls_configure_server_context(&e->ptls_ctx) != 0) {
        fprintf(stderr, "[ioxide.quic] ngtcp2_crypto_picotls_configure_server_context failed\n");
        goto fail;
    }
