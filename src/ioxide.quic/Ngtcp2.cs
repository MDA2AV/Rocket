...
/// <summary>
/// P/Invoke surface of the bundled native engine: the ioxide.quic shim (native/ioxide_quic_shim.c)
/// over ngtcp2 + its picotls crypto backend, linked into one shared library
/// (scripts/build-quic-native.sh) whose only external dependency is libcrypto.so.3. The shim owns
/// every ngtcp2/picotls struct layout in C, so this surface stays small and stable.
/// </summary>
internal static unsafe class Ngtcp2
{
    // Resolves runtimes/linux-x64/native/libioxide_quic.so from the package (or beside the app for
