...
public static unsafe class Native {
    ...
    // SQPOLL: kernel spawns a poller thread that reads SQEs from shared memory
    // and submits them without us calling io_uring_enter. SQ_AFF pins the poller
    // to a specific CPU. After sq_thread_idle ms of inactivity, the poller
    // parks and sets SQ_NEED_WAKEUP in sq_flags; we must then call
    // io_uring_enter(IORING_ENTER_SQ_WAKEUP) to revive it.
    public const uint   IORING_SETUP_SQPOLL        = 1u << 1;
    public const uint   IORING_SETUP_SQ_AFF        = 1u << 2;
    public const uint   IORING_SQ_NEED_WAKEUP      = 1u << 0;
