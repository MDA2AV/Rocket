...
using static KestrelMinima.Native;
...
public sealed unsafe partial class Reactor
{
    ...
    /// <summary>Set by the Kestrel transport: vend each accepted connection instead of running an inline handler.</summary>
    public Action<Connection>? OnAccept;
    ...
    private readonly Mpsc<int>    _flushQ  = new(1 << 12);   // 4096 slots (fire-and-forget hand-off from off-reactor FlushAsync)
    ...
    {
        // Fast path: caller is the reactor thread; submit the SQE for the new
        // bytes [WriteSubmitted, WriteTail) directly.
        if (Environment.CurrentManagedThreadId == _reactorThreadId)
        {
            if (Connections.TryGetValue(fd, out var conn))
            {
                SubmitSendRange(fd, conn);
            }
            return;
        }
        SpinWait sw = default;
    ...
    private void DrainFlushQ()
    {
        while (_flushQ.TryDequeue(out int fd))
        {
            ...
            // Mpsc Enqueue/Dequeue establishes happens-before, so WriteTail is
            // visible here.
            SubmitSendRange(fd, conn);
        }
    }

    // Submit a send for the new bytes the connection has produced since the last
    // SubmitSend. Reactor-thread only — mutates WriteSubmitted.
    private void SubmitSendRange(int fd, Connection conn)
    {
        int begin = conn.WriteSubmitted;
        int end   = conn.WriteTail;
        if (end <= begin)
        {
            return;
        }
        SubmitSend(fd, conn.WriteBuffer + begin, (uint)(end - begin));
        conn.WriteSubmitted = end;
    }
    ...
    public void Run()
    {
        ...
        Console.WriteLine($"[kestrel-minima r{Id}] listening on 0.0.0.0:{_port}");
    ...
    private void Dispatch(in IoUringCqe cqe)
    {
        ...
        if (kind == KindAccept)
        {
            if (cqe.res >= 0)
            {
                ...
                conn.InitInputPipe();        // recv lands in a BCL pipe Kestrel reads
        ...
        else if (kind == KindRecv)
        {
            ...
            // Kestrel: copy recv bytes into the BCL pipe, return the buffer.
            if (hasBuf)
            {
                conn.FeedInput(_bufSlab + (nuint)bid * (nuint)RecvBufferSize, cqe.res);
                ReturnBufferDirect(bid);
            }
        ...
        else if (kind == KindSend)
        {
            ...
            if (conn.WriteHead < conn.WriteSubmitted)
            {
                // Partial send: resubmit the remainder of the most recent batch.
                SubmitSend(fd, conn.WriteBuffer + conn.WriteHead, (uint)(conn.WriteSubmitted - conn.WriteHead));
                return;
            }
            // All submitted bytes ack'd. Reset only when Kestrel has produced
            // nothing further; otherwise the next DrainFlushQ pass picks up the
            // pending bytes.
            if (conn.WriteHead == conn.WriteTail)
            {
                conn.CompleteFlush();
            }
        }
    ...
    {
        // Kestrel transport: the connection is owned by the off-reactor consumer.
        // Signal EOF to its input pipe, close the fd, and drop it. Do NOT Clear/pool —
        // a recycled connection would race the consumer.
        conn.CompleteInput();
        conn.MarkClosed();
        close(fd);
    }
