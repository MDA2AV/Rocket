...
/// <summary>
/// Pure C# io_uring instance. Replaces the entire shim_* API surface.
/// Owns the ring fd and mmap'd memory regions.
/// </summary>
internal sealed unsafe class IoUring : IDisposable
{
    ...
    public static IoUring Create(uint entries, uint flags = 0, int sqCpu = -1, uint sqIdleMs = 100)
    {
        ...
        if ((flags & Constants.IORING_SETUP_SQPOLL) != 0)
        {
            if (sqCpu >= 0)
            {
                ...
                p.flags |= Constants.IORING_SETUP_SQ_AFF;
            }
            p.sq_thread_idle = sqIdleMs;
        }

        int fd = Syscalls.io_uring_setup(entries, &p);
        if (fd < 0)
        ...
        // mmap SQ+CQ ring
        void* sqRingMem = Syscalls.mmap(
            null, ringSize,
            Constants.PROT_READ | Constants.PROT_WRITE,
            Constants.MAP_SHARED | Constants.MAP_POPULATE,
            fd, Constants.IORING_OFF_SQ_RING);
        ...
        void* sqeMem = Syscalls.mmap(
            null, sqeSize,
            Constants.PROT_READ | Constants.PROT_WRITE,
            Constants.MAP_SHARED | Constants.MAP_POPULATE,
            fd, Constants.IORING_OFF_SQES);

        if (sqeMem == (void*)-1)
        {
            Syscalls.munmap(sqRingMem, ringSize);
            Syscalls.close(fd);
    ...
    public int Submit()
    {
        ...
        return Syscalls.io_uring_enter(_ringFd, toSubmit, 0, 0, null, 0);
    }
    ...
    public int SubmitAndWaitTimeout(uint waitNr, KernelTimespec* ts)
    {
        ...
        return Syscalls.io_uring_enter(
            _ringFd, toSubmit, waitNr,
            Constants.IORING_ENTER_GETEVENTS | Constants.IORING_ENTER_EXT_ARG,
    ...
    public void Dispose()
    {
        if (_sqRingPtr != null)
        {
            Syscalls.munmap(_sqRingPtr, _sqRingSize);
            _sqRingPtr = null;
        }

        if (_sqePtr != null)
        {
            Syscalls.munmap(_sqePtr, _sqeSize);
            _sqePtr = null;
        }

        if (_ringFd >= 0)
        {
            Syscalls.close(_ringFd);
            _ringFd = -1;
        }
    }
}
