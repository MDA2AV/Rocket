...
public sealed unsafe partial class Connection : IValueTaskSource, IBufferWriter<byte>
{
    ...
    {
        RunContinuationsAsynchronously = true,   // EXPERIMENT: resume flush continuation on the pool
    };
...
#region IBufferWrite<byte>
    ...
    public ValueTask FlushAsync()
    {
        ...
        // Submit the send directly from THIS (pool) thread — no queue, no reactor hand-off. Safe
        // because Reactor.SubmitSend takes the ring lock.
        _reactor.SubmitSend(ClientFd, WriteBuffer, (uint)WriteInFlight);
