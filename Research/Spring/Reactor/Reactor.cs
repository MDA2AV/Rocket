...
public sealed unsafe partial class Reactor
{
    ...
    private readonly Mpsc<int>    _flushQ  = new(1 << 12);   // 4096 slots (only used for the rare EAGAIN remainder)
    ...
    {
        // Only reached on EAGAIN (socket send buffer full): hand the remaining
        // bytes to the reactor (the ring's sole submitter), which sends them via
        // io_uring from the connection's current WriteHead.
        if (Environment.CurrentManagedThreadId == _reactorThreadId)
        {
    ...
    private void DrainFlushQ()
    {
        while (_flushQ.TryDequeue(out int fd))
        {
            ...
            // EAGAIN remainder: send from the current WriteHead (the pool thread's
            // inline send() already pushed everything up to WriteHead).
            SubmitSend(fd, conn.WriteBuffer + conn.WriteHead, (uint)(conn.WriteInFlight - conn.WriteHead));
        }
    }
    ...
    private void Recycle(Connection conn, int fd)
    {
        ...
        // Signal EOF to its input pipe, wake any flush awaiter, close the fd, and
        // drop it (GC's finalizer frees native). Do NOT Clear/pool — that resets
        // the flush IVTS Kestrel may still be using off-reactor, and a recycled
        // connection would race the consumer.
        if (_incremental)
        {
