...
public sealed unsafe class Ring : IDisposable 
{
    ...
    public static Ring Create(uint entries) 
    {
        ...
        // Single-issuer: only the reactor submits (accept/recv + the rare EAGAIN
        // remainder send via the eventfd handoff). Responses are sent with plain
        // send() OFF the ring, so we keep these fast-path flags (DEFER_TASKRUN
        // batches completion processing and requires SINGLE_ISSUER).
