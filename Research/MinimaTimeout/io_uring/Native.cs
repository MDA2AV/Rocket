...
public static unsafe class Native {
    ...
    public const uint IORING_ENTER_EXT_ARG   = 1u << 3;  // enter2: last 2 args are (getevents_arg*, size)
    ...
    [DllImport("libc", EntryPoint = "syscall", SetLastError = true)]
    private static extern long syscall6e(long nr, uint a1, uint a2, uint a3, uint a4, void* a5, nuint a6);
    ...
    // Extended form: with IORING_ENTER_EXT_ARG set, the last two syscall args are a
    // pointer to io_uring_getevents_arg (carrying a __kernel_timespec) and its size.
    // glibc's syscall() returns -1 and sets errno on failure (notably ETIME when the
    // timed wait expires), so translate back to a negative errno — callers rely on
    // -ETIME to distinguish a benign timeout from a real error.
    public static int io_uring_enter2(int fd, uint toSubmit, uint minComplete, uint flags, void* arg, nuint argSz)
    {
        long r = syscall6e(SYS_IO_URING_ENTER, (uint)fd, toSubmit, minComplete, flags, arg, argSz);
    ...
    // struct __kernel_timespec — seconds + nanoseconds, both 64-bit signed.
    [StructLayout(LayoutKind.Sequential)]
    ...
    // struct io_uring_getevents_arg (24 bytes) — extended wait argument. `ts` is a
    // pointer to a __kernel_timespec; a null sigmask skips signal-mask handling.
    [StructLayout(LayoutKind.Sequential)]
