...
public sealed unsafe class Ring : IDisposable 
{
    ...
    // Submit pending SQEs and wait for up to `waitFor` CQEs, but no longer than
    // `timeoutNs`. This is zerg's reactor-wake strategy: instead of an eventfd that
    // off-reactor threads poke, the reactor bounds its wait, so it loops back to
    // drain the MPSC queues on its own. Returns -ETIME (62) when the timeout fires.
    public int SubmitAndWaitTimeout(uint waitFor, long timeoutNs)
    {
        uint published = *_sqTail;
        uint toSubmit  = _sqeTail - published;
        ...
        io_uring_getevents_arg arg = default;
        arg.ts = (ulong)&ts;

        uint flags = IORING_ENTER_GETEVENTS | IORING_ENTER_EXT_ARG;
        return io_uring_enter2(_fd, toSubmit, waitFor, flags, &arg, (nuint)sizeof(io_uring_getevents_arg));
    }
