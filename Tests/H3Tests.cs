...
/// <summary>
/// The full HTTP/3 stack end to end: the ioxide reactor runs ngtcp2 + the ioxide.h3 (nghttp3)
...
internal static class H3Tests
{
    ...
    {
        runner.Test("h3: GET request/response through ioxide.h3 (loopback)", () =>
        {
...
/// <summary>
/// A minimal HTTP/3 client for the test: QUIC via the ioxide.quic shim's client entry points (like
/// QuicTestClient), H3 framing via the ioxide.h3 shim's client conn. Not production code - it
...
internal sealed unsafe class H3TestClient : IDisposable
{
    ...
    public (int Status, string Body) Get(string path, int timeoutMs)
    {
        ...
        {
            Assert.True(ih3_submit_request(_h3, _requestSid, p, (nuint)headers.Length) == 0,
    ...
    private void WriteStream(long sid, ReadOnlySpan<byte> data, bool fin)
    {
        ...
        fixed (byte* src = data)
        {
            ...
            while (true)
            {
                byte* dataPtr = id >= 0 ? src + off : null;
                nuint dataLen = id >= 0 ? (nuint)(data.Length - off) : 0;
                nint n = iq_conn_write(_conn, dest, (nuint)_sendScratch.Length, id,
                                       dataPtr, dataLen, fin ? 1 : 0, &consumed, NowNs());
                int code = (int)n;
                if (code < 0)
                {
                    id = -1;
    ...
    private const string QuicLib = "ioxide_quic";
    ...
    private const string H3Lib = "ioxide_h3";
    ...
    [DllImport(H3Lib)] private static extern int  ih3_submit_request(nint conn, long streamId, byte* headers, nuint headersLen);
